...
const Login = (props) => {
  ...
  const handleOpenErrorMessage = (providerName, errorMessage) => {
    if (errorMessage) {
      ...
      setErrorMessage(
        `Unable to log in via ${providerName}. Please try another method.`
      );
    }
  ...
  const handleResponseFacebook = async (facebookResponse) => {
    ...
    const userDidNotCompleteFacebookLogin =
      facebookResponse.status === "unknown";

    if (userDidNotCompleteFacebookLogin) {
      // TODO: write handler for this case too
      handleOpenErrorMessage(
        "Facebook",
        "You are not complete the auth via Facebook."
      );
      return;
    }
    try {
