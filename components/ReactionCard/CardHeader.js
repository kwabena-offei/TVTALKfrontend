...
const ReactionsCardHeader = ({
  ...
  const baseUrl =
    publicRuntimeConfig.API_ENV === "development"
      ? TV_TALK_HOST_LOCAL
      : TV_TALK_HOST;
  const copyLink = header
    ? `${baseUrl}${router.asPath}`
    : `${baseUrl}${router.asPath}#${id}`;
  ...
  return (
    <ShareContext.Provider value={{ openShare, setOpenShare }}>
      <CardHeader
        ...
        sx={
          isMobile
            ? cardHeaderMobileProps
            : { paddingX: 3.75, paddingY: 2.5, alignItems: "center" }
        }
        avatar={
          <Avatar
            sx={
              isMobile
                ? { width: 40, height: 40 }
                : { width: 60, height: 60, border: "1.5px solid #090F27" }
            }
        ...
        onClick={() => router.push(`/users/${username}/reactions`)}
      />
      <Share
        open={openShare}
        onClose={toggleShare}
